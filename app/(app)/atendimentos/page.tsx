...
import { atendimentos, getConsultorio, getPaciente, getProfissional } from '@/lib/mock/data';
...
function AtendimentosPageInner() {
  ...
  const filtrados = isProfissional ? atendimentos.filter((a) => a.profissionalId === profissionalId) : atendimentos;
  const ordenados = [...filtrados].sort((a, b) => `${b.data}T${b.hora}`.localeCompare(`${a.data}T${a.hora}`));
  const { page, totalPages, setPage, slice } = usePagination(ordenados.length);
  const visiveis = slice(ordenados);
              ...
              {visiveis.map((a) => {
                ...
                return (
                  <TableRow
                    ...
                    role="link"
                    tabIndex={0}
                    aria-label={`Ver atendimento de ${paciente?.nome ?? a.id}`}
