...
type ModoPeriodo = "semana" | "mes" | "custom";
...
export default function DashboardPage() {
  ...
  const [customInicio, setCustomInicio] = useState(semanaAtual.inicio);
  const [customFim, setCustomFim] = useState(semanaAtual.fim);

  const periodo = useMemo(() => {
    if (modo === "semana") return semanaAtual;
    if (modo === "mes") return mesAtual;
    return { inicio: customInicio, fim: customFim };
  }, [modo, semanaAtual, mesAtual, customInicio, customFim]);
        ...
        <CardContent className="space-y-4 p-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="inline-flex rounded-xl border border-border bg-card p-1 text-sm">
              {(["semana", "mes", "custom"] as ModoPeriodo[]).map((m) => (
                <button
                  key={m}
                  type="button"
                  onClick={() => setModo(m)}
