...
export default function TurnosPage() {
  ...
  return (
    <>
      ...
      <PageHeader
        title="Turnos"
        description="Faixas horárias da clínica — geram os horários disponíveis no agendamento e definem o cálculo de repasse aluguel-fixo"
      />
          ...
          return (
            <Card key={id}>
              <CardHeader className="flex flex-row items-center justify-between gap-2">
                ...
                {podeEditar && (
                  <Button size="sm" variant="ghost" onClick={() => setEditingId(id)}>
                    <Pencil size={14} />
                  </Button>
                )}
              </CardHeader>
      ...
      {editingId && editingTurno && <EditarTurnoDialog id={editingId} inicio={editingTurno.inicio} fim={editingTurno.fim} onClose={() => setEditingId(null)} onSave={(inicio, fim) => salvar(editingId, inicio, fim)} />}
    </>
  );
}

function EditarTurnoDialog({ id, inicio: inicioInicial, fim: fimInicial, onClose, onSave }: { id: TurnoId; inicio: string; fim: string; onClose: () => void; onSave: (inicio: string, fim: string) => void }) {
    ...
    <div role="dialog" aria-modal="true" className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md rounded-2xl border border-border bg-card p-6 shadow-xl">
        <div className="mb-4 flex items-start justify-between gap-3">
          <div>
            <h2 className="text-base font-semibold">Editar turno {TURNO_LABEL[id]}</h2>
            <p className="mt-0.5 text-xs text-muted-foreground">A alteração afeta o cálculo de repasse aluguel-fixo a partir de agora.</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <Input id="inicio" type="time" value={inicio} onChange={(e) => setInicio(e.target.value)} required />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="fim">Fim</Label>
              <Input id="fim" type="time" value={fim} onChange={(e) => setFim(e.target.value)} required />
            </div>
          </div>
